...
import { getCustomerApi } from "@/api/api";
...
import { updateSidebarNavigation,setKeForMessage } from "@/features/agentFeatures";
...
const NewCustomers = () => {
  ...
  const [dataView, setDataView] = useState<Item | undefined>(undefined);
  ...
  useEffect(() => {
    dispatch(updateSidebarNavigation("customers"));
  }, []);
  ...
  const fn_getProduct = async (pageValue: number, searchText: string) => {
    ...
    if (response?.data?.status === "ok") {
      ...
      // Define the function to find the object with the maximum 'orderPerDay'
      const findMaxOrderPerDay = (data: Item[]) => {
        if (!Array.isArray(data) || data.length === 0) return null; // Safety check for empty array

        // Use reduce to find the object with the greatest 'orderPerDay'
        const maxOrderObject = data.reduce((max, current) => {
          return Number(current.orderPerDay) > Number(max.orderPerDay)
            ? current
            : max;
        });

        return maxOrderObject;
      };

      // Get the object with the maximum 'orderPerDay'
      const maxOrderObject = findMaxOrderPerDay(data);

      // Log or use the maxOrderObject as needed
      if (maxOrderObject?.orderPerDay) {
        setMax(+maxOrderObject?.orderPerDay);
      }
  ...
  const changeRank = async (rank: string, productId: string) => {
    console.log(rank);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="font-[400] text-[14px]">
                            <>
                              {item?.imageLink ? (
                                <a href={item?.imageLink} target="__blank">
                                  {item?.imageLink?.slice(0, 27)}...
                                </a>
                              ) : (
                                <a href={item?.image} target="__blank" className="mx-auto  w-16 h-12 overflow-hidden flex justify-center items-center">
                                  <Image
                                    src={`${item?.image}`}
                                    alt="Product Image"
                          ...
                          <td className="align-middle text-sm text-center font-normal px-6 py-1.5">
                            <div className="flex flex-col gap-1">
                              ...
                              title="Working on"
                                ...
                                onClick={() => changeRank("working", item._id)}
                              ></button>
                              <button
                              title="Irrelevant"
                                className="w-4 h-4 bg-[#D71A217A] rounded-full"
                                onClick={() =>
                                  changeRank("irrelevant", item._id)
                                }
                              ></button>
                              <button
                              title="done"
                                className="w-4 h-4 bg-[#00FF1A] rounded-full"
                                onClick={() => changeRank("done", item._id)}
                              ></button>
                            </div>
                          </td>
                        </tr>
